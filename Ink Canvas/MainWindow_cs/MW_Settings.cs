    ...
    public partial class MainWindow : Window
    {
        ...
        private void ToggleSwitchRunAtStartup_Toggled(object sender, RoutedEventArgs e)
        {
            ...
            if (ToggleSwitchRunAtStartup.IsOn)
            {
                ...
                StartAutomaticallyCreate("Ink Canvas DeepRethink");
            }
            else
            {
                StartAutomaticallyDel("InkCanvas");
                StartAutomaticallyDel("Ink Canvas Annotation");
                StartAutomaticallyDel("Ink Canvas DeepRethink");
            }
        }
        ...
        private void ToggleSwitchAutoSaveStrokesAtScreenshot_Toggled(object sender, RoutedEventArgs e)
        {
            ...
            ToggleSwitchAutoSaveStrokesAtClear.Header = ToggleSwitchAutoSaveStrokesAtScreenshot.IsOn ? "错误ʱ�Զ���ͼ错误��ī��" : "错误ʱ�Զ���ͼ";
            SaveSettingsToFile();
        }
        ...
        private void BorderCalculateMultiplier_TouchDown(object sender, TouchEventArgs e)
        {
            ...
            else value = Math.Sqrt(args.Width * args.Height); //�ıߺ���
        ...
        private async void ToggleSwitchIsSuperTopmostEnabled_Toggled(object sender, RoutedEventArgs e)
        {
            if (!isLoaded) return;

            if (ToggleSwitchIsSuperTopmostEnabled.IsOn)
            {
                string nl = Environment.NewLine;
                string msg = "超级置顶功能需要配置 UIAccess 权限才能生效。" + nl + nl
                    + "配置步骤：" + nl
                    + "1. 软件会以管理员权限运行一个辅助程序" + nl
                    + "2. 辅助程序会将软件复制到 Program Files 目录" + nl
                    + "3. 设置 uiAccess=true 权限清单" + nl
                    + "4. 重启软件以应用设置" + nl + nl
                    + "注意：软件本身不会以管理员权限运行，PPT COM 联动不受影响。" + nl + nl
                    + "是否继续配置？";
                var result = MessageBox.Show(msg, "超级置顶设置", MessageBoxButton.YesNo, MessageBoxImage.Question);
                ...
                {
                    ToggleSwitchIsSuperTopmostEnabled.IsOn = false;
                    Settings.Advanced.IsSuperTopmostEnabled = false;
                    SaveSettingsToFile();
                    return;
                }
            }
            ...
            Settings.Advanced.IsSuperTopmostEnabled = ToggleSwitchIsSuperTopmostEnabled.IsOn;
            SaveSettingsToFile();
        }

        #endregion

        #region Super Topmost

        private void SetupSuperTopmost()
        {
            try
            {
                ...
                string targetDir = System.IO.Path.Combine(programFiles, "Ink Canvas DeepRethink");

                // 生成 PowerShell 配置脚本（使用 @ 逐字字符串 + 字符串拼接，避免 C# 换行符和插值问题）
                string nl = Environment.NewLine;
                string psScript = string.Join(nl,
                    "# UIAccess 配置脚本 - Ink Canvas DeepRethink",
                    "$ErrorActionPreference = 'Stop'",
                    "",
                    "$sourceDir = '" + appDir + "'",
                    "$targetDir = '" + targetDir + "'",
                    "$exeName = 'Ink Canvas DeepRethink.exe'",
                    "",
                    "# 1. 错误Ŀ��Ŀ¼",
                    ...
                    "}",
                    "",
                    "# 2. 错误错误�ļ��� Program Files",
                    "Copy-Item -Path (Join-Path $sourceDir '*') -Destination $targetDir -Recurse -Force",
                    "",
                    "# 3. �޸��嵥�ļ��е� uiAccess Ϊ true",
                    "$manifestPath = Join-Path $targetDir 'app.manifest'",
                    "$manifest = Get-Content $manifestPath -Raw",
                    "$manifest = $manifest -replace 'uiAccess=\"false\"', 'uiAccess=\"true\"'",
                    "Set-Content -Path $manifestPath -Value $manifest -Encoding UTF8",
                    "",
                    "# 4. 错误��ʼ�˵���ݷ�ʽ",
                    "$shortcutPath = [System.IO.Path]::Combine([Environment]::GetFolderPath('Programs'), 'Ink Canvas DeepRethink.lnk')",
                    "$wshShell = New-Object -ComObject WScript.Shell",
                    "$shortcut = $wshShell.CreateShortcut($shortcutPath)",
                    "$shortcut.TargetPath = Join-Path $targetDir $exeName",
                    "$shortcut.WorkingDirectory = $targetDir",
                    "$shortcut.Description = 'Ink Canvas DeepRethink 错误'",
                    "$shortcut.Save()",
                    "",
                    "Write-Host '错误��ɣ���ӿ�ʼ�˵错误 Ink Canvas DeepRethink ��ʹ�ó错误ö错误ܡ�'",
                    "Write-Host '��ǰʵ错误��ȷ错误رգ错误ֶ错误错误��'",
                    "Read-Host '�� Enter ���˳�'"
                );

                string psPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "SetupSuperTopmost.ps1");
                System.IO.File.WriteAllText(psPath, psScript, System.Text.Encoding.UTF8);
                ...
                string msg = "配置程序已以管理员权限启动。" + nl + nl
                    + "请完成以下步骤：" + nl
                    + "1. 等待 PowerShell 窗口中的配置脚本执行完毕" + nl
                    + "2. 从开始菜单启动 Ink Canvas DeepRethink" + nl
                    + "3. 重新打开此开关以启用超级置顶" + nl + nl
                    + "注意：如果提示数字签名问题，可以尝试使用工具给 exe 签名后再试。";
                MessageBox.Show(msg, "超级置顶设置", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                LogHelper.WriteLogToFile("SetupSuperTopmost failed: " + ex, LogHelper.LogType.Error);
                MessageBox.Show("配置超级置顶时出错：" + ex.Message, "错误", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        private void ApplyTopmostNormal()
        {
            this.Topmost = false;
            this.Topmost = true;
        }
