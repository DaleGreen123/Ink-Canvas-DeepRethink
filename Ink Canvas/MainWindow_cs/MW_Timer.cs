...
namespace Ink_Canvas
{
    public partial class MainWindow : Window
    {
        ...
        /// <summary>
        /// 定时检测触摸屏设备数量变化。
        /// 当触摸屏被移除再重新接入时，Tablet.TabletDevices 数量会先减少再恢复，
        /// 借此触发触控子系统重置，与 SystemEvents.DisplaySettingsChanged 形成双重保障。
        /// </summary>
        private void TimerCheckTouchDevices_Elapsed(object sender, ElapsedEventArgs e)
        {
            try
            {
                int currentCount = System.Windows.Input.Tablet.TabletDevices.Count;
                if (lastTabletDeviceCount != -1 && lastTabletDeviceCount != currentCount)
                {
                    Dispatcher.Invoke(() => OnTabletDeviceCountChanged());
                }
                lastTabletDeviceCount = currentCount;
            }
            catch { }
        }
    }
